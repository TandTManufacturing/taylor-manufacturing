...
function Button({ children, className = "", size, variant, ...props }) {
  return (
    <button
      className={`inline-flex items-center justify-center font-semibold transition ${size === "lg" ? "h-12 text-base" : "h-10 text-sm"} ${className}`}
      {...props}
    >
      {children}
    </button>
...
function Icon({ label, className = "" }) {
  return <span className={`inline-flex items-center justify-center ${className}`}>{label}</span>;
}

const ArrowRight = ({ className = "" }) => <Icon label="→" className={className} />;
const CheckCircle2 = ({ className = "" }) => <Icon label="✓" className={className} />;
const Factory = ({ className = "" }) => <Icon label="⚙" className={className} />;
const Gauge = ({ className = "" }) => <Icon label="◉" className={className} />;
const ShieldCheck = ({ className = "" }) => <Icon label="✓" className={className} />;
const Clock = ({ className = "" }) => <Icon label="◷" className={className} />;
const Mail = ({ className = "" }) => <Icon label="✉" className={className} />;
const Phone = ({ className = "" }) => <Icon label="☎" className={className} />;
const MapPin = ({ className = "" }) => <Icon label="⌖" className={className} />;
const capabilities = [
  ...
  return (
    <div className="min-h-screen bg-[#f3f3ef] text-zinc-900">
      <header className="sticky top-0 z-50 border-b border-zinc-300 bg-[#f3f3ef]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <nav className="hidden items-center gap-8 text-sm text-zinc-700 md:flex">
            <a href="#capabilities" className="hover:text-zinc-900">Capabilities</a>
            <a href="#quality" className="hover:text-zinc-900">Quality</a>
            <a href="#industries" className="hover:text-zinc-900">Industries</a>
            <a href="#contact" className="hover:text-zinc-900">Contact</a>
          </nav>
          <Button className="rounded-2xl bg-white px-5 text-zinc-950 hover:bg-zinc-200">
            Email RFQ
          </Button>
        </div>
      </header>
      ...
      <main className="bg-[#f3f3ef] text-zinc-900">
          ...
          <div className="relative mx-auto grid max-w-[1600px] items-center gap-20 px-8 py-20 lg:grid-cols-[1.2fr_0.8fr] lg:py-28">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
              ...
              <h1 className="max-w-4xl text-5xl font-semibold  tracking-[-0.03em] text-zinc-900 md:text-7xl">
                High-performance manufacturing built around precision and quality.
              </h1>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-zinc-700">
                Taylor Manufacturing delivers precision manufacturing and engineering support for motorsports, aerospace, industrial, and prototype applications with a strong emphasis on quality, inspection, and repeatability.
              </p>
              <div className="mt-9 flex flex-col gap-4 sm:flex-row">
                <Button size="lg" className="rounded-2xl bg-white px-7 text-zinc-950 hover:bg-zinc-200">
                  Start an RFQ <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
                <Button size="lg" variant="outline" className="rounded-2xl border-white/20 bg-white/70 px-7 text-zinc-900 hover:bg-white/70">
                  View Capabilities
                </Button>
              </div>
              <div className="mt-10 grid gap-4 text-sm text-zinc-700 sm:grid-cols-3">
                <div className="rounded-2xl border border-zinc-300 bg-white/70 p-4">
                  <p className="text-2xl font-medium text-zinc-900">Precision</p>
                  <p>Automated quality-focused manufacturing</p>
                </div>
                <div className="rounded-2xl border border-zinc-300 bg-white/70 p-4">
                  <p className="text-2xl font-medium text-zinc-900">Quality</p>
                  <p>In process part inspection, attention to detail on every surface</p>
                </div>
                <div className="rounded-2xl border border-zinc-300 bg-white/70 p-4">
              ...
              <div className="relative overflow-hidden rounded-[2.5rem] border border-zinc-300 shadow-none">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-8">
                  <div className="flex items-center gap-3 text-zinc-700">
                    ...
                    <p className="text-sm uppercase tracking-[0.3em]">QUALITY • PROCESS CONTROL • PRECISION</p>
                  </div>
                  <h3 className="mt-4 text-3xl font-semibold  leading-tight text-zinc-900">
                    Precision manufacturing driven by quality, process control, and repeatability. No Shortcuts. No Compromises
                  </h3>
                </div>
              </div>
            </motion.div>
          ...
          <div className="grid lg:grid-cols-2">
            ...
            <div className="flex items-center bg-[#ecece7] px-12 py-20 lg:px-20">
              <div className="max-w-2xl">
                <p className="text-sm uppercase tracking-[0.3em] text-zinc-400">Capabilities</p>
                <h2 className="mt-6 text-5xl font-semibold tracking-tight text-zinc-900">
                  Engineering-driven manufacturing built around quality and repeatability.
                </h2>

                <div className="mt-14 space-y-6 border-t border-zinc-300 pt-10 text-xl text-zinc-700">
                  {capabilities.map((item) => (
                    <div key={item} className="flex items-center justify-between border-b border-zinc-300 pb-5">
                      <span>{item}</span>
        ...
        <section id="quality" className="border-top border-zinc-300 bg-[#efefe9]">
          <div className="grid lg:grid-cols-[1fr_1.1fr]">
            <div className="flex items-center px-10 py-24 lg:px-20">
              <div className="max-w-2xl">
                <p className="text-sm uppercase tracking-[0.3em] text-zinc-400">Quality</p>
                <h2 className="mt-6 text-5xl font-semibold tracking-tight text-zinc-900">
                ...
                <div className="mt-10 space-y-8 text-lg leading-9 text-zinc-700">
                  <p>
                    Every component begins with print review, process planning, tolerance evaluation, and a defined inspection strategy before machining begins.
                  </p>

                  <p>
                    Inspection, traceability, and process control are treated as part of the manufacturing workflow.
                  </p>
                </div>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-px bg-white/70 min-h-[700px]">
              <img
        ...
        <section id="industries" className="border-t border-zinc-300 bg-[#f3f3ef]">
          <div className="px-10 py-24 lg:px-20">
            <p className="text-sm uppercase tracking-[0.3em] text-zinc-400">Industries</p>
            <h2 className="mt-6 max-w-4xl text-5xl font-semibold tracking-tight text-zinc-900">
              Engineering and manufacturing support for high-performance and mission-critical industries.
            </h2>
          </div>

          <div className="grid lg:grid-cols-3">
            <div className="relative h-[520px] overflow-hidden border-t border-zinc-300">
              <img src="/images/aerospace.jpg" alt="Aerospace" className="h-full w-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
              <div className="absolute bottom-0 p-10">
                <p className="text-3xl font-semibold text-white">Aerospace & Defense</p>
              </div>
            </div>

            <div className="relative h-[520px] overflow-hidden border-t border-zinc-300 lg:border-l">
              <img src="/images/motorsports.webp" alt="Motorsports" className="h-full w-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
              <div className="absolute bottom-0 p-10">
                <p className="text-3xl font-semibold text-white">Motorsports</p>
              </div>
            </div>

            <div className="relative h-[520px] overflow-hidden border-t border-zinc-300 lg:border-l">
              <img src="/images/industrial.jpg" alt="Industrial" className="h-full w-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
              <div className="absolute bottom-0 p-10">
                <p className="text-3xl font-semibold text-white">Industrial & Prototype</p>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="overflow-hidden rounded-[1.25rem] border border-zinc-300 bg-[#f5f5f3] text-zinc-950 shadow-none">
            <div className="grid gap-0 lg:grid-cols-[1fr_0.8fr]">
              <div className="p-8 md:p-12">
                <p className="text-sm font-medium uppercase tracking-[0.3em] text-zinc-400">Request a Quote</p>
                <h2 className="mt-4 text-4xl font-semibold tracking-tight md:text-5xl">Send your print. Get a clear path forward.</h2>
                <p className="mt-5 max-w-2xl text-lg leading-8 text-zinc-600">Include a PDF print, STEP file, material, quantity, finish requirements, tolerance requirements, and target delivery date.</p>
                <div className="mt-8 flex flex-col gap-4 sm:flex-row">
                  <Button size="lg" className="rounded-2xl bg-zinc-950 px-7 text-white hover:bg-zinc-800">
                    Email RFQ <Mail className="ml-2 h-4 w-4" />
                  </Button>
                  <Button size="lg" variant="outline" className="rounded-2xl border-zinc-300 px-7 text-zinc-950 hover:bg-zinc-50">
                    Capabilities Sheet
                  </Button>
                </div>
              </div>
              <div id="contact" className="bg-zinc-100 p-8 md:p-12">
                ...
                <div className="mt-6 space-y-5 text-zinc-700">
                  <div className="flex items-center gap-3"><Mail className="h-5 w-5" /> quotes@taylormanufacturingus.com</div>
                  <div className="flex items-center gap-3"><Phone className="h-5 w-5" /> (520) 981-9556</div>
      ...
      <footer className="border-t border-zinc-300 px-6 py-8 text-center text-sm text-zinc-400">
        © {new Date().getFullYear()} Taylor Manufacturing • TaylorManufacturingUS.com
      </footer>
    </div>
  );
}
